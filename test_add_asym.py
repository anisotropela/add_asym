...
def add_asym(x0s,siglos,sighis,order=2,nmax=100,tol=1e-6,ohwell=False):
  ...
  if order==0:                                  #   separately in quadrature  |
    if not ohwell: print('This is WRONG! Mark my words, it is WROOOOOONG!!!')#|
    x0    = np.sum(x0s)                         #                             |
    siglo = sqrt(np.sum(siglos**2))             #                             |
    sighi = sqrt(np.sum(sighis**2))             #_____________________________|

  else:
    # ------------------   First, calculate total moments   -------------------
    mu,V,gamma = 0.,0.,0.                             #                       |
    if order==1:                                      #Cumulate mean,var,skew |
      sig    = (sighis+siglos) / 2.                   #"The mean"      }_eq. 1|
      alpha  = (sighis-siglos) / 2.                   #"The difference"}      |
      ...
      gamma  = np.sum(6*sig**2 * alpha + 8*alpha**3)  #Skewness_______________|
    ...
    # -----------  Use moments to find resulting distribution  ----------------
      ...
      if order==1:                              #                             |
        D = 0.                                  #Iterator; equal to sighi-siglo
        while True:                             #                             |
          n   += 1                              #Update counter               |
...
def plotTests(order,nn='all',
  ...
  Here, "some method" refers the standard, but wrong, quadrature addition
  (order 0), or the add_asym() method (order 1 or 2).
  ...
  fname = '/Users/pela/Python/add_asym/add_asym_'+x0+'_normdDiffs.dat'
...
def showSkews(n=1,skew=[0,10],png=False):
  """
  Draw 'n' figures with skewness in 'skew' range.
  The plot window may be resized manually first.

  Example:
  --------
  To create 5 png files with skewness in [0,1]:
  >>> showSkews(n=5,skew=[0,1],png=True)
  ...
  while i<n:
    ...
    S = skewness(P,x)
    if not (skew[0] <= S <= skew[1]): continue
    i += 1
    print(i)
    s = f.fflt(S)
